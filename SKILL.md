...
VTXOs are offchain Bitcoin outputs managed by an Ark server. Each VTXO has:
- An owner (identified by their public key)
- An amount in satoshis
- An expiry time (after which the server can sweep unclaimed funds)
- A taproot script tree defining spending conditions
...
1. **MultisigClosure**: Requires signatures from all specified public keys
2. **CLTVMultisigClosure**: Multisig that can only be spent after an absolute time/block
3. **CSVMultisigClosure**: Multisig with relative timelock (e.g., "24 hours after confirmation")
4. **ConditionMultisigClosure**: Multisig plus a custom condition (e.g., hash preimage)
5. **ConditionCSVMultisigClosure**: Combines CSV timelock with custom condition
